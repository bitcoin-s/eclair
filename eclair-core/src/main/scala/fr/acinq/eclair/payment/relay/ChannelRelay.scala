...
object ChannelRelay {
  ...
  private case class WrappedForwardShortIdFailure(failure: Register.ForwardShortIdFailure[AddCommand]) extends Command
  private case class WrappedAddResponse(res: CommandResponse[AddCommand]) extends Command
  // @formatter:on

  // @formatter:off
  sealed trait RelayResult
  case class RelayFailure(cmdFail: FailCommand) extends RelayResult
  case class RelaySuccess(shortChannelId: ShortChannelId, cmdAdd: AddCommand) extends RelayResult
  // @formatter:on
...
class ChannelRelay private(nodeParams: NodeParams,
  ...
  private val forwardShortIdAdapter = context.messageAdapter[Register.ForwardShortIdFailure[AddCommand]](WrappedForwardShortIdFailure)
  ...
  def waitForAddResponse(selectedShortChannelId: ShortChannelId, previousFailures: Seq[PreviouslyTried]): Behavior[Command] =
    Behaviors.receiveMessagePartial {
      case WrappedForwardShortIdFailure(Register.ForwardShortIdFailure(Register.ForwardShortId(_, shortChannelId, CMD_ADD_PTLC(_, _, _, _, _, _, o: Origin.ChannelRelayedHot, _)))) =>
        context.log.warn(s"couldn't resolve downstream channel $shortChannelId, failing ptlc #${o.add.id}")
        val cmdFail = CMD_FAIL_PTLC(o.add.id, Right(UnknownNextPeer), commit = true)
        Metrics.recordPaymentRelayFailed(Tags.FailureType(cmdFail), Tags.RelayType.Channel)
        safeSendAndStop(o.add.channelId, cmdFail)

      case WrappedAddResponse(addFailed@RES_ADD_FAILED(CMD_ADD_PTLC(_, _, _, _, _, _, _: Origin.ChannelRelayedHot, _), _, _)) =>
  ...
  def waitForAddSettled(): Behavior[Command] =
    Behaviors.receiveMessagePartial {
      case WrappedAddResponse(RES_ADD_SETTLED(o: Origin.ChannelRelayedHot, htlc: UpdateAddHtlc, fulfill: HtlcResult.Fulfill)) =>
      ...
      case WrappedAddResponse(RES_ADD_SETTLED(o: Origin.ChannelRelayedHot, ptlc: UpdateAddPtlc, fulfill: HtlcResult.RemoteFulfillPtlc)) =>
        context.log.info("relaying fulfill to upstream")
        val cmd = CMD_FULFILL_PTLC(o.originHtlcId, fulfill.nextPaymentPreimage, commit = true)
        context.system.eventStream ! EventStream.Publish(ChannelPaymentRelayed(o.amountIn, o.amountOut, ptlc.paymentHash, o.originChannelId, ptlc.channelId))
        safeSendAndStop(o.originChannelId, cmd)

      case WrappedAddResponse(RES_ADD_SETTLED(o: Origin.ChannelRelayedHot, ptlc: UpdateAddPtlc, fulfill: HtlcResult.FulfillPtlc)) =>
        context.log.info("relaying fulfill to upstream")
   ...
   */
  def handleRelay(previousFailures: Seq[PreviouslyTried]): RelayResult = r.add match {
    case _: UpdateAddHtlc => handleRelayHtlc(previousFailures)
    case _: UpdateAddPtlc => handleRelayPtlc(previousFailures)
  }

  def handleRelayHtlc(previousFailures: Seq[PreviouslyTried]): RelayResult = {
  ...
  def handleRelayPtlc(previousFailures: Seq[PreviouslyTried]): RelayResult = {
    val alreadyTried = previousFailures.map(_.shortChannelId)
    selectPreferredChannel(alreadyTried)
      .flatMap(selectedShortChannelId => channels.get(selectedShortChannelId).map(_.channelUpdate)) match {
      case None if previousFailures.nonEmpty =>
        ...
        val error = previousFailures
          ...
          .getOrElse(previousFailures.head)
          .failure
        RelayFailure(CMD_FAIL_PTLC(r.add.id, Right(translateLocalError(error.t, error.channelUpdate)), commit = true))
      case channelUpdate_opt =>
        relayOrFailPtlc(channelUpdate_opt)
    }
  }
  ...
  def relayOrFailPtlc(channelUpdate_opt: Option[ChannelUpdate]): RelayResult = {
    import r._
    add match {
          ...
          case Some(channelUpdate) =>
            payload.nextPointTweak match {
              case Some(pointTweak) =>
                ...
                RelaySuccess(channelUpdate.shortChannelId, CMD_ADD_PTLC(addResponseAdapter.toClassic, payload.amountToForward, ptlcKeys, paymentPoint, payload.outgoingCltv, nextPacket, origin, commit = true))
              case None =>
                RelayFailure(CMD_FAIL_PTLC(add.id, Right(IncorrectOrUnknownPaymentDetails(add.amountMsat, 0)), commit = true))
            }
        }
      case _ => RelayFailure(CMD_FAIL_PTLC(add.id, Right(InvalidRealm), commit = true))
    }
  }
