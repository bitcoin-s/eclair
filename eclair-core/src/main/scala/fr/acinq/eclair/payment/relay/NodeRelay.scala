...
import fr.acinq.eclair.payment.send.PaymentLifecycle.SendPaymentHtlc
...
class NodeRelay private(nodeParams: NodeParams,
    ...
    Behaviors.receiveMessagePartial {
      ...
      case Relay(IncomingPacket.NodeRelayPacket(add, outer, inner, next)) => outer.paymentSecret match {
        ...
        case Some(secret) =>
          ...
          mppFsm ! MultiPartPaymentFSM.HtlcPart(outer.totalAmount, add, None)
   ...
   */
  private def receiving(htlcs: Queue[UpdateAddMessage], secret: ByteVector32, nextPayload: Onion.NodeRelayPayload, nextPacket: OnionRoutingPacket, handler: ActorRef): Behavior[Command] =
    Behaviors.receiveMessagePartial {
      case Relay(IncomingPacket.NodeRelayPacket(add, outer, _, _)) => outer.paymentSecret match {
        ...
        case Some(incomingSecret) if incomingSecret == secret =>
          context.log.debug("forwarding incoming htlc to the payment FSM")
          handler ! MultiPartPaymentFSM.HtlcPart(outer.totalAmount, add, None)
      ...
      case WrappedMultiPartPaymentFailed(MultiPartPaymentFSM.MultiPartPaymentFailed(_, failure, parts)) =>
        ...
        parts.collect { case p: MultiPartPaymentFSM.HtlcPart => rejectHtlc(p.add.id, p.add.channelId, p.amount, Some(failure)) }
        Behaviors.stopped
      ...
      case Some(features) =>
        ...
        payloadOut.paymentSecret match {
          ...
          case _ =>
            ...
            val payment = SendPaymentHtlc(payFsmAdapters, payloadOut.outgoingNodeId, finalPayload, nodeParams.maxPaymentAttempts, routingHints, Some(routeParams))
  ...
  private def rejectExtraHtlcPartialFunction: PartialFunction[Command, Behavior[Command]] = {
    ...
    case WrappedMultiPartExtraPaymentReceived(extraPaymentReceived) =>
      rejectExtraHtlc(extraPaymentReceived.payment.add)
      Behaviors.same
  }

  private def rejectExtraHtlc(add: UpdateAddMessage): Unit = {
