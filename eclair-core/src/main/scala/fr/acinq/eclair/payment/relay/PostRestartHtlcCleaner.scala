...
import fr.acinq.eclair.transactions.{IncomingHtlc, IncomingTlc}
import fr.acinq.eclair.wire.{FailureMessage, TemporaryNodeFailure, UpdateAddHtlc, UpdateAddMessage}
...
class PostRestartHtlcCleaner(nodeParams: NodeParams, register: ActorRef, initialized: Option[Promise[Done]] = None) extends Actor with ActorLogging {
  ...
  val brokenHtlcs: BrokenHtlcs = {
    ...
    val nonStandardIncomingHtlcs = nodeParams.pluginParams.collect { case p: CustomCommitmentsPlugin => p.getIncomingHtlcs }.flatten
    val htlcsIn = getIncomingHtlcs(channels, nodeParams.db.payments, nodeParams.privateKey) ++ nonStandardIncomingHtlcs
    ...
    case e@ChannelStateChanged(channel, channelId, _, _, WAIT_FOR_INIT_INTERNAL | OFFLINE | SYNCING | CLOSING, NORMAL | SHUTDOWN | CLOSING | CLOSED, Some(commitments: AbstractCommitments)) =>
      ...
      val acked = brokenHtlcs.notRelayed
        ...
        .filter {
          ...
          case IncomingUpdateAddMessage(htlc, preimage_opt) if commitments.getIncomingHtlcCrossSigned(htlc.id).isDefined =>
...
object PostRestartHtlcCleaner {
  ...
  def decryptedIncomingHtlcs(paymentsDb: IncomingPaymentsDb): PartialFunction[Either[FailureMessage, IncomingPacket], IncomingUpdateAddMessage] = {
  ...
  private def getIncomingHtlcs(channels: Seq[HasCommitments], paymentsDb: IncomingPaymentsDb, privateKey: PrivateKey)(implicit log: LoggingAdapter): Seq[IncomingUpdateAddMessage] = {
  ...
  private def isPendingUpstream(channelId: ByteVector32, htlcId: Long, htlcsIn: Seq[IncomingUpdateAddMessage]): Boolean =
  ...
  private def getHtlcsRelayedOut(channels: Seq[HasCommitments], htlcsIn: Seq[IncomingUpdateAddMessage])(implicit log: LoggingAdapter): Map[Origin, Set[(ByteVector32, Long)]] = {
    channels
      .flatMap { c =>
   ...
   */
  private def cleanupRelayDb(htlcsIn: Seq[IncomingUpdateAddMessage], relayDb: PendingRelayDb)(implicit log: LoggingAdapter): Unit = {
    // We are interested in incoming HTLCs, that have been *cross-signed* (otherwise they wouldn't have been relayed).
    ...
    val channel2Htlc: Seq[(ByteVector32, Long)] = htlcsIn.map { htlc =>  (htlc.add.channelId, htlc.add.id) }
