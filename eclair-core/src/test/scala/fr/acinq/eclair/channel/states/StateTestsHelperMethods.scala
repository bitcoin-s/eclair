...
trait StateTestsHelperMethods extends TestKitBase with FixtureTestSuite with ParallelTestExecution {
  ...
  def makeCmdAddPtlc(amount: MilliSatoshi, destination: PublicKey, currentBlockHeight: Long, paymentPoint: PublicKey = randomKey.publicKey, nextPointTweak: PrivateKey = randomKey, upstream: Upstream = Upstream.Local(UUID.randomUUID), replyTo: ActorRef = ActorRef.noSender): (PublicKey, PrivateKey, CMD_ADD_PTLC) = {
    val expiry = CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight)
