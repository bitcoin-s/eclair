...
class PtlcCommitmentsSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with StateTestsHelperMethods {
  ...
  test("correct values for availableForSend/availableForReceive (success case)") { f =>
    ...
    val (payment_point, _, cmdAdd) = makeCmdAddPtlc(p, bob.underlyingActor.nodeParams.nodeId, currentBlockHeight, paymentPoint = paymentPoint + tweak.publicKey)
    val Right((ac1, add)) = sendAddPtlc(ac0, cmdAdd, currentBlockHeight, alice.underlyingActor.nodeParams.onChainFeeConf)
  ...
  test("correct values for availableForSend/availableForReceive (failure case)") { f =>
    ...
    val (_, _, cmdAdd) = makeCmdAddPtlc(p, bob.underlyingActor.nodeParams.nodeId, currentBlockHeight, paymentPoint = paymentPoint + tweak.publicKey)
    val Right((ac1, add)) = sendAddPtlc(ac0, cmdAdd, currentBlockHeight, alice.underlyingActor.nodeParams.onChainFeeConf)
    assert(ac1.availableBalanceForSend == a - p - fee) // as soon as htlc is sent, alice sees its balance decrease (more than the payment amount because of the commitment fees)
    assert(ac1.availableBalanceForReceive == b)
  ...
  test("correct values for availableForSend/availableForReceive (multiple htlcs)") { f =>
    ...
    val (payment_point1, _, cmdAdd1) = makeCmdAddPtlc(p1, bob.underlyingActor.nodeParams.nodeId, currentBlockHeight, paymentPoint = paymentPoint1 + tweak1.publicKey)
    ...
    val (_, _, cmdAdd2) = makeCmdAddPtlc(p2, bob.underlyingActor.nodeParams.nodeId, currentBlockHeight, paymentPoint = paymentPoint2 + tweak2.publicKey)
    ...
    val (payment_point3, _, cmdAdd3) = makeCmdAddPtlc(p3, alice.underlyingActor.nodeParams.nodeId, currentBlockHeight, paymentPoint = paymentPoint3 + tweak3.publicKey)
