...
import fr.acinq.eclair.payment.OutgoingPacket.{Upstream, buildCommand, buildCommandPtlc, buildOnion}
...
class RelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("relay a trampoline htlc-add at the final node to the payment handler") { f =>
    ...
    val (trampolineAmount, trampolineExpiry, trampolineOnion) = OutgoingPacket.buildPacket(Sphinx.TrampolinePacket)(Some(paymentHash), trampolineHops, Onion.createMultiPartPayload(finalAmount, totalAmount, finalExpiry, paymentSecret))
  ...
  test("fail to relay a trampoline htlc-add when trampoline is disabled", Tag("trampoline-disabled")) { f =>
    ...
    val (trampolineAmount, trampolineExpiry, trampolineOnion) = OutgoingPacket.buildPacket(Sphinx.TrampolinePacket)(Some(paymentHash), trampolineHops, Onion.createSinglePartPayload(finalAmount, finalExpiry))
    val (cmd, _) = buildCommand(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, ChannelHop(a, b, channelUpdate_ab) :: Nil, Onion.createTrampolinePayload(trampolineAmount, trampolineAmount, trampolineExpiry, randomBytes32, trampolineOnion.packet))
  ...
  test("relay an ptlc-add") { f =>
    ...
    register.expectMessageType[Register.ForwardShortId[CMD_ADD_PTLC]]
  }
  ...
  test("relay ptlc settled") { f =>
    import f._

    val replyTo = TestProbe[Any]()
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 42, amountMsat = 11000000 msat, paymentPoint_ab, CltvExpiry(4200), TestConstants.emptyOnionPacket)
    val add_bc = UpdateAddPtlc(channelId_bc, 72, 1000 msat, paymentPoint_bc, CltvExpiry(1), TestConstants.emptyOnionPacket)
    val channelOrigin = Origin.ChannelRelayedHot(replyTo.ref.toClassic, add_ab, 1000 msat)
    val trampolineOrigin = Origin.TrampolineRelayedHot(replyTo.ref.toClassic, Seq(add_ab))
