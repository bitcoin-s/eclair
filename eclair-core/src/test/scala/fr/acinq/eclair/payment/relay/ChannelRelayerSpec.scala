...
import fr.acinq.eclair.payment.PaymentPacketSpec.paymentPoint
...
class ChannelRelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  def expectFwdAddPtlc(register: TestProbe[Any], shortChannelId: ShortChannelId, outAmount: MilliSatoshi, outExpiry: CltvExpiry): Register.ForwardShortId[CMD_ADD_PTLC] = {
    val fwd = register.expectMessageType[Register.ForwardShortId[CMD_ADD_PTLC]]
    assert(fwd.shortChannelId === shortChannelId)
    assert(fwd.message.amount === outAmount)
    assert(fwd.message.cltvExpiry === outExpiry)
    assert(fwd.message.origin.isInstanceOf[Origin.ChannelRelayedHot])
  ...
  test("relay ptlc-add") { f =>
    import f._
    import fr.acinq.eclair.wire.OnionTlv._

    val payload = ChannelRelayTlvPayload(TlvStream[OnionTlv](AmountToForward(outgoingAmount), OutgoingCltv(outgoingExpiry), OutgoingChannelId(shortId1), NextPointTweak(randomKey)))
    val r = createValidIncomingPacketPtlc(1100000 msat, CltvExpiry(400100), payload)
    val u = createLocalUpdate(shortId1)

    channelRelayer ! WrappedLocalChannelUpdate(u)
    channelRelayer ! Relay(r)

    expectFwdAddPtlc(register, shortId1, outgoingAmount, outgoingExpiry)
  }
  ...
  test("relay an ptlc-add with retries") { f =>
    ...
    // first try
    val fwd1 = expectFwdAddPtlc(register, shortId2, outgoingAmount, outgoingExpiry)
    ...
    // second try
    val fwd2 = expectFwdAddPtlc(register, shortId1, outgoingAmount, outgoingExpiry)
    // failure again
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_PTLC(r.add.id, Right(TemporaryNodeFailure), commit = true))
  }

  test("fail to relay an ptlc-add when we have no point tweak ") { f =>
    import f._
    import fr.acinq.eclair.wire.OnionTlv._
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_PTLC(r.add.id, Right(IncorrectOrUnknownPaymentDetails(1100000 msat, 0)), commit = true))
  }
...
object ChannelRelayerSpec {
  ...
  def createValidIncomingPacketPtlc(amountIn: MilliSatoshi, expiryIn: CltvExpiry, payload: ChannelRelayPayload): IncomingPacket.ChannelRelayPacket = {
    val add_ab = UpdateAddPtlc(channelId = randomBytes32, id = 123456, amountIn, paymentPoint, expiryIn, emptyOnionPacket)
    ChannelRelayPacket(add_ab, payload, emptyOnionPacket)
  }
