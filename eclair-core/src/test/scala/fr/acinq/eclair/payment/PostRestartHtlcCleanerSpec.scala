...
import fr.acinq.eclair.payment.relay.PostRestartHtlcCleaner.IncomingUpdateAddMessage
...
class PostRestartHtlcCleanerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with StateTestsHelperMethods {
  ...
  test("Relayed nonstandard->standard HTLC is retained") { f =>
    ...
    val pluginParams = new CustomCommitmentsPlugin {
      ...
      def getIncomingHtlcs: Seq[IncomingUpdateAddMessage] = List(IncomingUpdateAddMessage(relayedHtlc1In.add, None), IncomingUpdateAddMessage(nonRelayedHtlc2In.add, None))
      def getHtlcsRelayedOut(htlcsIn: Seq[IncomingUpdateAddMessage]): Map[Origin, Set[(ByteVector32, Long)]] = Map.empty
    }
    ...
    val cs = new AbstractCommitments {
      ...
      def getOutgoingPtlcCrossSigned(htlcId: Long): Option[UpdateAddPtlc] = None
      def getIncomingPtlcCrossSigned(htlcId: Long): Option[UpdateAddPtlc] = None
      def timedOutOutgoingHtlcs(blockheight: Long): Set[UpdateAddMessage] = Set.empty
  ...
  test("Relayed standard->nonstandard HTLC is retained") { f =>
    ...
    val pluginParams = new CustomCommitmentsPlugin {
      ...
      def getIncomingHtlcs: Seq[IncomingUpdateAddMessage] = List.empty
      def getHtlcsRelayedOut(htlcsIn: Seq[IncomingUpdateAddMessage]): Map[Origin, Set[(ByteVector32, Long)]] = Map(trampolineRelayed -> Set((channelId_ab_1, 10L)))
    }
  ...
  test("Non-standard HTLC CMD_FAIL in relayDb is retained") { f =>
    ...
    val pluginParams = new CustomCommitmentsPlugin {
      def name = "test with incoming HTLC from remote"
      def getIncomingHtlcs: Seq[IncomingUpdateAddMessage] = List(IncomingUpdateAddMessage(relayedHtlc1In.add, None))
      def getHtlcsRelayedOut(htlcsIn: Seq[IncomingUpdateAddMessage]): Map[Origin, Set[(ByteVector32, Long)]] = Map.empty
    }
