...
class RelayerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("relay an htlc-add with onion tlv payload") { f =>
    ...
    val Sphinx.PacketAndSecrets(onion, _) = buildOnion(Sphinx.PaymentPacket)(hops.map(_.nextNodeId), payloads, Some(paymentHash))
  ...
  test("relay an ptlc-add with retries") { f =>
    ...
    // we use this to build a valid onion
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, hops, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), PTLCData(finalPointTweak))))
    // and then manually build an htlc
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.paymentPoint, cmd.cltvExpiry, cmd.onion)
  ...
  test("relay a trampoline htlc-add with retries") { f =>
    ...
    val (trampolineAmount, trampolineExpiry, trampolineOnion) = buildPacket(Sphinx.TrampolinePacket)(Some(paymentHash), trampolineHops, Onion.createMultiPartPayload(finalAmount, totalAmount, finalExpiry, paymentSecret))
  ...
  test("relay an ptlc-add at the final node to the payment handler") { f =>
    ...
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, hops.take(1), pointTweaks.take(1), FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), PTLCData(finalPointTweak))))
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.paymentPoint, cmd.cltvExpiry, cmd.onion)
    sender.send(relayer, RelayForward(add_ab))
  ...
  test("fail to relay an ptlc-add when the channel is advertised as unusable (down)") { f =>
    ...
    val (cmd1, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), randomKey.publicKey, hops, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), PTLCData(finalPointTweak))))
    val add_ab1 = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd1.amount, cmd1.paymentPoint, cmd1.cltvExpiry, cmd1.onion)
    sender.send(relayer, RelayForward(add_ab1))
  ...
  test("fail to relay an ptlc-add when the onion is malformed") { f =>
    ...
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, hops, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), PTLCData(finalPointTweak))))
    // and then manually build an htlc with an invalid onion (hmac)
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.paymentPoint, cmd.cltvExpiry, cmd.onion.copy(hmac = cmd.onion.hmac.reverse))
    relayer ! LocalChannelUpdate(null, channelId_bc, channelUpdate_bc.shortChannelId, c, None, channelUpdate_bc, makeCommitments(channelId_bc))
  ...
  test("fail to relay a trampoline htlc-add when trampoline is disabled") { f =>
    ...
    val (trampolineAmount, trampolineExpiry, trampolineOnion) = buildPacket(Sphinx.TrampolinePacket)(Some(paymentHash), trampolineHops, Onion.createSinglePartPayload(finalAmount, finalExpiry))
    val (cmd, _) = buildCommand(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, ChannelHop(a, b, channelUpdate_ab) :: Nil, Onion.createTrampolinePayload(trampolineAmount, trampolineAmount, trampolineExpiry, randomBytes32, trampolineOnion.packet))
  ...
  test("fail to relay an ptlc-add when amount is below the next hop's requirements") { f =>
    ...
    val zeroFeeHops = hops.map(hop => hop.copy(lastUpdate = hop.lastUpdate.copy(feeBaseMsat = 0 msat, feeProportionalMillionths = 0)))
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, zeroFeeHops, pointTweaks, finalPayload)
    // and then manually build an htlc
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.paymentPoint, cmd.cltvExpiry, cmd.onion)
    relayer ! LocalChannelUpdate(null, channelId_bc, channelUpdate_bc.shortChannelId, c, None, channelUpdate_bc, makeCommitments(channelId_bc))
  ...
  test("fail to relay an ptlc-add when expiry does not match next hop's requirements") { f =>
    ...
    val hops1 = hops.updated(1, hops(1).copy(lastUpdate = hops(1).lastUpdate.copy(cltvExpiryDelta = CltvExpiryDelta(0))))
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, hops1, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(channelUpdate_bc.htlcMinimumMsat - (1 msat)), OutgoingCltv(finalExpiry), PTLCData(finalPointTweak))))
    // and then manually build an htlc
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.paymentPoint, cmd.cltvExpiry, cmd.onion)
    relayer ! LocalChannelUpdate(null, channelId_bc, channelUpdate_bc.shortChannelId, c, None, channelUpdate_bc, makeCommitments(channelId_bc))
  ...
  test("correctly translates errors returned by channel when attempting to add an ptlc") { f =>
    ...
    val origin = Origin.ChannelRelayedHot(channelRelayer, upstreamAdd,1000000.msat)
    val cmdAdd = CMD_ADD_PTLC(channelRelayer,1000000.msat, paymentPoint, CltvExpiry(144), TestConstants.emptyOnionPacket, origin)
  ...
  def testRelayTrampolineHtlcFulfill(f: FixtureParam, onChain: Boolean): Unit = {
    ...
    val trampolineHops = NodeHop(a, b, channelUpdate_ab.cltvExpiryDelta, 0 msat) :: NodeHop(b, c, nodeParams.expiryDelta, nodeFee(nodeParams.feeBase, nodeParams.feeProportionalMillionth, finalAmount)) :: Nil
    val (trampolineAmount, trampolineExpiry, trampolineOnion) = buildPacket(Sphinx.TrampolinePacket)(Some(paymentHash), trampolineHops, Onion.createMultiPartPayload(finalAmount, finalAmount, finalExpiry, paymentSecret))
    val secret_ab = randomBytes32
