...
class RelayerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("relay an htlc-add with onion tlv payload") { f =>
   ...
   test("relay an ptlc-add") { f =>
    ...
    // Use tlv payloads for all hops (final and intermediate)
    val finalPayload: Seq[PerHopPayload] = FinalTlvPayload(TlvStream[OnionTlv](AmountToForward(finalAmount), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak))) :: Nil
    ...
    val (firstAmountMsat, firstExpiry, payloads) = hops.zip(pointTweaks).drop(1).reverse.foldLeft((finalAmount, finalExpiry, finalPayload)) {
      case ((amountMsat, expiry, currentPayloads), (hop, tweak)) =>
        val nextFee = nodeFee(hop.lastUpdate.feeBaseMsat, hop.lastUpdate.feeProportionalMillionths, amountMsat)
        val payload = ChannelRelayTlvPayload(TlvStream[OnionTlv](AmountToForward(amountMsat), OutgoingCltv(expiry), OutgoingChannelId(hop.lastUpdate.shortChannelId), NextPointTweak(tweak)))
        (amountMsat + nextFee, expiry + hop.lastUpdate.cltvExpiryDelta, payload +: currentPayloads)
    }
  ...
  test("relay an ptlc-add with retries") { f =>
    ...
    // we use this to build a valid onion
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, pointTweak, finalPaymentPoint, hops, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak))))
    // and then manually build an htlc
  ...
  test("relay an ptlc-add at the final node to the payment handler") { f =>
    ...
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, pointTweak, finalPaymentPoint, hops.take(1), pointTweaks.take(1), FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak))))
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.nextPaymentPoint, cmd.cltvExpiry, cmd.onion)
    sender.send(relayer, RelayForward(add_ab))

    val fp = paymentHandler.expectMsgType[FinalPacket]
    assert(fp.add === add_ab)
    assert(fp.payload === FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak))))
  ...
  test("fail to relay an ptlc-add when the channel is advertised as unusable (down)") { f =>
    ...
    val (cmd1, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, pointTweak, finalPaymentPoint, hops, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(finalAmount), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak))))
  ...
  test("fail to relay an ptlc-add when amount is below the next hop's requirements") { f =>
    ...
    // we use this to build a valid onion
    val finalPayload = FinalTlvPayload(TlvStream(AmountToForward(channelUpdate_bc.htlcMinimumMsat - (1 msat)), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak)))
    val zeroFeeHops = hops.map(hop => hop.copy(lastUpdate = hop.lastUpdate.copy(feeBaseMsat = 0 msat, feeProportionalMillionths = 0)))
  ...
  test("fail to relay an ptlc-add when expiry does not match next hop's requirements") { f =>
    ...
    val hops1 = hops.updated(1, hops(1).copy(lastUpdate = hops(1).lastUpdate.copy(cltvExpiryDelta = CltvExpiryDelta(0))))
    val (cmd, _) = buildCommandPtlc(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentPoint, pointTweak, finalPaymentPoint, hops1, pointTweaks, FinalTlvPayload(TlvStream(AmountToForward(channelUpdate_bc.htlcMinimumMsat - (1 msat)), OutgoingCltv(finalExpiry), NextPointTweak(finalPointTweak))))
    // and then manually build an htlc
    val add_ab = UpdateAddPtlc(channelId = channelId_ab, id = 123456, cmd.amount, cmd.nextPaymentPoint, cmd.cltvExpiry, cmd.onion)
    relayer ! LocalChannelUpdate(null, channelId_bc, channelUpdate_bc.shortChannelId, c, None, channelUpdate_bc, makeCommitments(channelId_bc))
  ...
  test("relay an ptlc-fulfill") { f =>
    ...
    val origin = Origin.ChannelRelayedHot(channelRelayer, add_ab, 10000000 msat)
    for (fulfill <- Seq(RES_ADD_SETTLED(origin, add_bc, HtlcResult.RemoteFulfillPtlc(fulfill_ba, randomKey)), RES_ADD_SETTLED(origin, add_bc, HtlcResult.OnChainFulfillPtlc(randomKey)))) {
      sender.send(relayer, fulfill)
