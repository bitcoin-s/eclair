...
import fr.acinq.eclair.payment.Monitoring.Tags.Directions.Received
...
class PtlcIntegrationSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  ignore("open a wumbo channel C <-> F1 and wait for longer than the default min_depth") {
  ...
  test("send an PTLC A->D") {
    ...
    val incomingPayment_opt = nodes("D").nodeParams.db.payments.getIncomingPayment(paymentHash)
  ...
  ignore("send an HTLC A->D with an amount greater than capacity of B-C") {
    val sender = TestProbe()
  ...
  ignore("send multiple HTLCs A->D with a failover when a channel gets exhausted") {
    val sender = TestProbe()
  ...
  ignore("send an HTLC A->B->G->C using heuristics to select the route") {
    val sender = TestProbe()
  ...
  ignore("send a multi-part payment B->D") {
  ...
  ignore("send a multi-part payment B->D greater than balance C->D (temporary remote failure)") {
    val sender = TestProbe()
  ...
  ignore("send a multi-part payment D->C (local channels only)") {
    val sender = TestProbe()
  ...
  ignore("send a multi-part payment D->C greater than balance D->C (temporary local failure)") {
    val sender = TestProbe()
  ...
  ignore("send a trampoline payment B->F1 with retry (via trampoline G)") {
    val start = System.currentTimeMillis
    val sender = TestProbe()
  ...
  ignore("send a trampoline payment D->B (via trampoline C)") {
    val start = System.currentTimeMillis
    val sender = TestProbe()
  ...
  ignore("send a trampoline payment F1->A (via trampoline C, non-trampoline recipient)") {
  ...
  ignore("send a trampoline payment B->D (temporary local failure at trampoline)") {
    val sender = TestProbe()
  ...
  ignore("send a trampoline payment A->D (temporary remote failure at trampoline)") {
    val sender = TestProbe()
  ...
  ignore("open channel C <-> F2, send payments and close (option_anchor_outputs, option_static_remotekey)") {
